...
List<List<int>> createBoard() =>
    List.generate(kRows, (_) => List.filled(kCols, kEmpty));
...
List<List<int>> copyBoard(List<List<int>> b) =>
    List.generate(kRows, (r) => List<int>.from(b[r]));

bool checkWin(List<List<int>> b, int p) {
  for (int r = 0; r < kRows; r++)
    for (int c = 0; c <= kCols - kWin; c++) {
      bool w = true;
      for (int i = 0; i < kWin; i++) if (b[r][c+i] != p) { w = false; break; }
      if (w) return true;
    }
  for (int c = 0; c < kCols; c++)
    for (int r = 0; r <= kRows - kWin; r++) {
      bool w = true;
      for (int i = 0; i < kWin; i++) if (b[r+i][c] != p) { w = false; break; }
      if (w) return true;
    }
  for (int r = 0; r <= kRows - kWin; r++)
    for (int c = 0; c <= kCols - kWin; c++) {
      bool w = true;
      for (int i = 0; i < kWin; i++) if (b[r+i][c+i] != p) { w = false; break; }
      if (w) return true;
    }
...
int _scoreBoard(List<List<int>> b, int p) {
  int score = 0;
  for (int r = 0; r < kRows; r++) if (b[r][kCols ~/ 2] == p) score += 3;
  for (int r = 0; r < kRows; r++)
    for (int c = 0; c <= kCols - kWin; c++)
...
_MM _minimax(List<List<int>> b, int depth, int alpha, int beta, bool max) {
  if (isTerminal(b)) {
  ...
  if (max) {
    ...
    for (final c in cols) {
      ...
      final s = _minimax(tmp, depth - 1, alpha, beta, false).score;
    ...
    for (final c in cols) {
      ...
      final s = _minimax(tmp, depth - 1, alpha, beta, true).score;
      if (s < best) { best = s; bc = c; }
...
int getBestCol(List<List<int>> b, int depth) =>
    _minimax(b, depth, -999999999, 999999999, true).col ?? validCols(b).first;
